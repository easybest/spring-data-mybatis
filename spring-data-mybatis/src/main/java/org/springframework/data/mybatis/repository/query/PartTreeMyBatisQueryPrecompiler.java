...
class PartTreeMyBatisQueryPrecompiler extends MybatisQueryMethodPrecompiler {
	...
	//
	// PartTreeMyBatisQueryPrecompiler(MybatisMappingContext mappingContext,
	...
	// sql += this.buildSelectStatementSegment(ResidentStatementName.UNPAGED_PREFIX +
	...
	// columns =
	// Stream.of(selectColumns).map(String::trim).filter(StringUtils::hasText).map(
	// sc ->
	// this.persistentEntity.getRequiredPersistentProperty(sc).getColumn().getName().render(dialect))
	...
	// return tree.stream().map(orPart -> new OrPart(orPart, this.persistentEntity,
	// this.mappingContext,
	// this.argumentCounter, this.query.getQueryMethod(),
	// this.dialect)).collect(Collectors.toList());
	// }
	//
	// @Override
	...
	//
	// OrPart(PartTree.OrPart or, MybatisPersistentEntity<?> persistentEntity,
	...
	// .map(part -> new AndPart(part, persistentEntity, mappingContext, argumentCounter,
	// method, dialect))
	...
	//
	// AndPart(Part part, MybatisPersistentEntity<?> persistentEntity,
	// MybatisMappingContext mappingContext,
	// AtomicInteger argumentCounter, MybatisQueryMethod method, Dialect dialect) {
	// MybatisPersistentProperty persistentProperty = persistentEntity
	// .getRequiredPersistentProperty(part.getProperty().getSegment());
	// if (persistentProperty.isAnnotationPresent(EmbeddedId.class) ||
	// persistentProperty.isEmbeddable()) {
	...
	// || (part.shouldIgnoreCase() == IgnoreCaseType.WHEN_POSSIBLE && null !=
	// persistentProperty
	...
	// this.arguments[i] = bindableParameter.getName().orElse("__p" +
	// (bindableParameter.getIndex() + 1));
