...
abstract class AbstractMybatisPrecompiler implements MybatisPrecompiler {
	...
	//
	// AbstractMybatisPrecompiler(MybatisMappingContext mappingContext,
	...
	// }
	//
	// AbstractMybatisPrecompiler(MybatisMappingContext mappingContext, Class<?>
	// domainType) {
	...
	// String path = "org/springframework/data/mybatis/repository/query/template/" + name
	// + ".mustache";
	...
	// this.getClass().getClassLoader().getResourceAsStream(path)) {
	// try (InputStreamReader source = new InputStreamReader(inputStream,
	...
	// XMLMapperBuilder mapperBuilder = new XMLMapperBuilder(inputStream,
	// this.configuration,
	// this.namespace.replace('.', '/') + UUID.randomUUID().toString() +
	// this.getResourceSuffix(),
	...
	// .render(this.dialect);
	// }
	// map.put(String.format("%s.%s", inverse.getName(),
	// targetEntity.getRequiredIdProperty().getName()),
	...
	// }
	//
	// protected List<MybatisPersistentProperty> findProperties(MybatisPersistentEntity<?>
	// entity) {
	// List<MybatisPersistentProperty> properties = new ArrayList<>();
	// entity.doWithProperties((PropertyHandler<MybatisPersistentProperty>)
	// properties::add);
	// return properties;
	// }
	//
	...
	// }
	//
	// protected String buildQueryByConditionLeftSegment(String column,
	...
	// }
	//
	// protected String buildQueryByConditionRightSegment(Part.Type type,
	// Part.IgnoreCaseType ignoreCaseType,
	...
	// "<foreach item=\"__item\" index=\"__index\" collection=\"%s\" open=\"(\"
	// separator=\",\" close=\")\">#{__item}</foreach>",
	// properties[0]);
	// case TRUE:
	// return " = 1";
	// case FALSE:
	// return " = 0";
	// default:
	// if (ignoreCaseType == Part.IgnoreCaseType.ALWAYS || ignoreCaseType ==
	...
	// }
	// return String.format("%s(#{%s})", this.dialect.getLowercaseFunction(),
	// properties[0]);
	// }
	// if (null != column.getTypeHandler()) {
	// return String.format("#{%s,javaType=%s,typeHandler=%s}", properties[0],
	// column.getJavaType().getName(),
	...
	// }
	//
	// protected String buildLikeRightSegment(String property, boolean left, boolean
	// right,
	// Part.IgnoreCaseType ignoreCaseType) {
	// return String.format("<bind name=\"__bind_%s\" value=\"%s%s%s\" />%s", property,
	// (left ? "'%' + " : ""),
	// property, (right ? " + '%'" : ""),
	...
	// String bind = String.format("<bind name=\"__columnsMap\" value='#{%s}'/>",
	// mapping);
	// String sql = String.format("order by "
	// + " <foreach collection=\"%s\" item=\"item\" index=\"idx\" open=\"\" close=\"\"
	// separator=\",\">"
	// + "<if test=\"item.ignoreCase\">%s(</if>" + "${__columnsMap[item.property]}"
	// + "<if test=\"item.ignoreCase\">)</if> " + "${item.direction.name().toLowerCase()}"
	// + "</foreach>",
	// ResidentParameterName.SORT, this.dialect.getLowercaseFunction());
	// return String.format("<if test=\"%s != null\">%s %s</if>",
	// ResidentParameterName.SORT, bind, sql);
	// }
