...
class SimpleMybatisPrecompiler extends AbstractMybatisPrecompiler {
	...
	//
	// SimpleMybatisPrecompiler(MybatisMappingContext mappingContext,
	...
	// "Are you forget to generate " + this.persistentEntity.getType().getName() +
	// "Example ?");
	// }
	//
	...
	// "The %s extends MybatisExampleRepository, but could not find @Example on the
	// entity: %s",
	...
	// return this.configuration.hasStatement(this.namespace +
	// SqlSessionRepositorySupport.DOT + statementName, false);
	// }
	//
	...
	// return "";
	// }
	// MybatisPersistentProperty idProperty =
	// this.persistentEntity.getRequiredIdProperty();
	//
	// Map<String, Object> scopes = new HashMap<>();
	...
	//
	// this.persistentEntity.doWithProperties((PropertyHandler<MybatisPersistentProperty>)
	// pp -> {
	...
	// this.persistentEntity.doWithAssociations((AssociationHandler<MybatisPersistentProperty>)
	// ass -> {
	// MybatisPersistentProperty inverse = ass.getInverse();
	// if (inverse.isAnnotationPresent(ManyToOne.class) ||
	// inverse.isAnnotationPresent(OneToOne.class)) {
	...
	//
	// Association association = new Association(inverse)
	...
	//// .setSelect(namespace + '.' + this.addAssociationManyToOne(namespace,
	// association));
	//
	// associations.add(association);
	// }
	//
	// if (inverse.isAnnotationPresent(ManyToMany.class) ||
	// inverse.isAnnotationPresent(OneToMany.class)) {
	// MybatisPersistentEntity<?> targetEntity = this.mappingContext
	// .getRequiredPersistentEntity(inverse.getActualType());
	//
	// String namespace = targetEntity.getType().getName();
	...
	// if (null != joinTable) {
	// Collection.JoinTable jt = new
	// Collection.JoinTable(StringUtils.hasText(joinTable.name())
	...
	// .render(this.dialect))));
	// }
	// collection.setSelect(namespace + '.' + this.addAssociationManyToMany(namespace,
	// collection));
	// }
	// else {
	...
	// // select statement.
	// if (this.configuration.hasResultMap(this.namespace + '.' +
	...
	// String statementName = "__association_to_one_" +
	// this.persistentEntity.getName().replace('.', '_') + "_"
	...
	// String statementName = "__association_many_to_many_" +
	// this.persistentEntity.getName().replace('.', '_') + "_"
	...
	// String statementName = "__association_one_to_many_" +
	// this.persistentEntity.getName().replace('.', '_') + "_"
	// + collection.getProperty();
	// Map<String, Object> scopes = new HashMap<>();
	// scopes.put("statementName", statementName);
	...
	// if (this.checkStatement(selective ? ResidentStatementName.INSERT_SELECTIVE :
	// ResidentStatementName.INSERT)) {
	// return "";
	// }
	//
	...
	//
	// Map<String, Object> scopes = new HashMap<>();
	// scopes.put("statementName", selective ? ResidentStatementName.INSERT_SELECTIVE :
	// ResidentStatementName.INSERT);
	// scopes.put("properties", this.mappingPropertyToColumn().entrySet());
	...
	// if (gv.strategy() == GenerationType.IDENTITY || (gv.strategy() ==
	// GenerationType.AUTO
	// && "identity".equals(this.dialect.getNativeIdentifierGeneratorStrategy()))) {
	// // identity
	// sql =
	// this.dialect.getIdentityColumnSupport().getIdentitySelectString(this.getTableName(),
	// idProperty.getColumn().getName().getCanonicalName(),
	// idProperty.getColumn().getJdbcType().TYPE_CODE);
	// executeBefore = false;
	// }
	// else if (gv.strategy() == GenerationType.SEQUENCE || (gv.strategy() ==
	// GenerationType.AUTO
	...
	// .collect(Collectors.toMap(SequenceGenerator::name,
	// SequenceGenerator::sequenceName)));
	// }
	// if (this.persistentEntity.isAnnotationPresent(SequenceGenerator.class)) {
	// SequenceGenerator sg =
	// this.persistentEntity.getRequiredAnnotation(SequenceGenerator.class);
	...
	// }
	// else {
	// throw new UnsupportedOperationException("unsupported generated value id strategy: "
	// + gv.strategy());
	// }
	...
	// ? (byId ? ResidentStatementName.UPDATE_SELECTIVE_BY_ID :
	...
	// if (this.checkStatement(ResidentStatementName.DELETE_BY_ID) || null ==
	// this.persistentEntity.getIdClass()) {
	// return "";
	// }
	//
	// Map<String, Object> scopes = new HashMap<>();
	...
	// if (this.checkStatement(ResidentStatementName.GET_BY_ID) || null ==
	// this.persistentEntity.getIdClass()) {
	// return "";
	// }
	// Map<String, Object> scopes = new HashMap<>();
	...
	// if (this.checkStatement(((pageable) ? ResidentStatementName.FIND_BY_PAGER :
	// ResidentStatementName.FIND))) {
	// return "";
	// }
	// Map<String, Object> scopes = new HashMap<>();
	// scopes.put("statementName", pageable ? ResidentStatementName.FIND_BY_PAGER :
	// ResidentStatementName.FIND);
	// scopes.put("resultMap", ResidentStatementName.RESULT_MAP);
	// scopes.put("table", this.getTableName());
	...
	// scopes.put("SQLServer2005", this.dialect.getLimitHandler().getClass() ==
	// SQLServer2005LimitHandler.class);
	// scopes.put("SQLServer2012", this.dialect.getLimitHandler().getClass() ==
	// SQLServer2012LimitHandler.class);
	// }
	...
	// ResidentStatementName.FIND_BY_EXAMPLE, false)) {
	// return "";
	// }
	//
	// Map<String, Object> scopes = new HashMap<>();
	...
	// if (this.configuration.getSqlFragments().containsKey(this.namespace +
	...
	// return (frag, out) -> out
	// .write(this.dialect.getLimitHandler().processSql(frag.execute(), new
	// RowSelection(true)));
	// }
	//
	...
	// prev = conditions[i];
	// }
	// String test = Stream.of(conditions).map(c -> c.trim() + (not ? " !" : " =") + "=
	// null")
	...
	// if (type.getNumberOfArguments() > 0 && type.getNumberOfArguments() !=
	// properties.length) {
	// throw new MappingException("@Condition with type " + type + " needs " +
	// type.getNumberOfArguments()
	// + " arguments, but only find " + properties.length + " properties in this
	// @Condition.");
	// }
	// String cond = Stream.of(properties).map(property -> String.format("__condition.%s
	// != null", property))
	// .collect(Collectors.joining(" AND "));
	// Part.IgnoreCaseType ignoreCaseType =
	// Part.IgnoreCaseType.valueOf(c.ignoreCaseType().name());
	// String columnName = StringUtils.hasText(c.column()) ? c.column()
	// : pp.getColumn().getName().render(this.dialect);
	// String left = this.buildQueryByConditionLeftSegment(columnName, ignoreCaseType,
	// pp);
	// String operator = this.buildQueryByConditionOperator(type);
	// String right = this.buildQueryByConditionRightSegment(type, ignoreCaseType,
	// properties, pp.getColumn());
	// return String.format("<if test=\"%s\"> AND %s %s %s</if>", cond, left, operator,
	// right);
	// }).collect(Collectors.joining());
	// }).collect(Collectors.joining());
	// return String.format("<if test=\"__condition != null\"><trim prefixOverrides=\"AND
	// |OR \">%s</trim></if>", sql);
	// }
