...
class SimpleMybatisQueryPrecompiler extends MybatisQueryMethodPrecompiler {
	...
	//
	// SimpleMybatisQueryPrecompiler(MybatisMappingContext mappingContext,
	...
	//
	// List<StringQuery.ParameterBinding> parameterBindings =
	...
	// separator=\",\" close=\")\">#{__item%s}</foreach>",
	...
	//
	// StringQuery.LikeParameterBinding likeParameterBinding =
	// (StringQuery.LikeParameterBinding) parameterBinding;
	//
	...
	// }
	//
	// sql = sql.replace(replace, String.format("<bind name=\"__bind_%s\" value=\"%s\" />
	// #{__bind_%s}",
	...
	//
	// String className = constructorExpression.substring(3,
	// constructorExpression.lastIndexOf("(")).trim();
	// Class<?> clz;
	// try {
	...
	// }
	// String columns =
	// constructorExpression.substring(constructorExpression.lastIndexOf("(") + 1,
	...
	// () -> new MappingException("Could not find constructor for: " +
	// constructorExpression));
	//
	// for (int i = 0; i < constructor.getParameterCount(); i++) {
	// Parameter parameter = constructor.getParameters()[i];
	// results.append(String.format("<arg column=\"%s\" javaType=\"%s\"/>",
	// columnList[i].trim(),
	// parameter.getType().getName()));
	// }
	//
	// results.append("</constructor>");
	// resultMap = ResidentStatementName.RESULT_MAP_PREFIX +
	// this.query.getStatementName();
	//
	// result = String.format("<resultMap id=\"%s\" type=\"%s\">%s</resultMap>",
	// resultMap, className,
	// results.toString());
	//
	...
	// }
	//
	// if (method.isPageQuery() || parameters.hasPageableParameter() ||
	// method.isSliceQuery()) {
	...
	// sql = this.dialect.getLimitHandler().processSql(sql + sort, new
	// RowSelection(true));
	// }
	//
	// String select = String.format("<select id=\"%s\" %s>%s</select>",
	// this.query.getStatementName(),
	...
	// return "_simple_" + this.query.getQueryMethod().getStatementName() +
	// super.getResourceSuffix();
	// }
