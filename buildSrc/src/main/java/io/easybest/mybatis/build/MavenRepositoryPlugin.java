...
public class MavenRepositoryPlugin implements Plugin<Project> {
	...
	public void apply(Project project) {
		...
		project.getTasks().matching(task -> task.getName().equals(PUBLISH_TO_PROJECT_REPOSITORY_TASK_NAME))
				.all(task -> this.setUpProjectRepository(project, task, repositoryLocation));
	...
	private void setUpProjectRepository(Project project, Task publishTask, File repositoryLocation) {
		...
		project.getPlugins().withType(JavaPlugin.class).all(javaPlugin -> this.addMavenRepositoryDependencies(project,
				JavaPlugin.IMPLEMENTATION_CONFIGURATION_NAME, target));
		project.getPlugins().withType(JavaLibraryPlugin.class).all(javaLibraryPlugin -> this
				.addMavenRepositoryDependencies(project, JavaPlugin.API_CONFIGURATION_NAME, target));
		project.getPlugins().withType(JavaPlatformPlugin.class).all(javaPlugin -> this
